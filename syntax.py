...
# 移进归约过程并将中间结果按格式输出到2gra.txt文件中
status_stack = deque([0])
symbol_stack = deque([])
input_list = deque(tokens)
step = 1
with open('2gra.txt', 'w', newline='') as f:
    ...
    while True:
        ...
        if table.loc[status, symbol] == 'acc':
            tsv_w.writerow([f'{top}#{symbol}', 'accept'])
            break
            ...
            input_list.popleft()
            tsv_w.writerow([f'{top}#{symbol}', 'move'])
